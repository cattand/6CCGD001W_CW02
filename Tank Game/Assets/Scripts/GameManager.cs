...
namespace TanksMP
{
	...
	public class GameManager : MonoBehaviour
	{
        ...
        public void DisplayGameOver1()
        {
            ...
            ui.SetGameOverText(teams[teamIndex], isADraw);

            if (localPlayer.myName == teams[teamIndex].name)
            {
                //LoginUI.GetInstance().UserAuthentication();

                GameObject lobbyManager = GameObject.Find("LobbyManager");

                int playerScore = lobbyManager.GetComponent<LobbyManager>().playerScore;
                ...
                //UpdateScore(playerScore.ToString());

                GameObject loginPanel = GameObject.Find("LoginPanel");

                //loginPanel.GetComponent<LoginUI>().UpdateScore(playerScore.ToString());

                LoginUI.GetInstance().UpdateScore(playerScore.ToString());

                //GameManager.GetInstance().localPlayer

                Debug.Log("Yay..");
            }
            else
            {
                Debug.Log("Dun be sad.." + localPlayer.name + " OMG - " + teams[teamIndex].name);
